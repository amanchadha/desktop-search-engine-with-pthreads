...
typedef struct{
  ...
  int maxSize; //Maximum size of the Bounded Buffer-Initialized in main()
  int head;
  int tail;
...
//Data Structure for the Reader Writer locks to be used for inserting and searching values in the hash table
typedef struct{
...
void *scanner_thread(void *file_list){
  ...
  char *filename = malloc(sizeof(char) * 513);
	//Counts the total number of files in list-of-files.txt
  totalFiles = 0;
...
void *indexer_thread(){
  ...
  while(1){
    //part 1
    pthread_mutex_lock(file_buffer->buffer_lock);
		//wait till Bounded Buffer is populated by producer
...
void *query_thread(void *input){
  ...
  int advancedFound = 0;//Used to differentiate between Basic and Advanced Searches
...
int main(int argc, char *argv[])
{
  ...
  int j;
  //waiting for indexer and query threads to finish
